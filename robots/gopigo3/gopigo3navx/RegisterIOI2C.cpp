...
bool RegisterIO_I2C::Read(uint8_t first_address, uint8_t* buffer, uint8_t buffer_len) {
	...
	while ( len > 0 ) {
        ...
        if (!port->Write(first_address + buffer_offset, static_cast<uint8_t>(read_len)) &&
	...
	while (buffer_len > 0)
	{
		int count = (buffer_len > MAX_WPILIB_I2C_READ_BYTES) ? MAX_WPILIB_I2C_READ_BYTES : buffer_len;
		if (!port->Read(first_address, count, buffer))
			return false;

		buffer_len = static_cast<uint8_t>(buffer_len - count;
		buffer += count;
		first_address = static_cast<uint8_t>(first_address + count) ;
	}
#else
	while (buffer_len-- > 0)
	{
		if (!port->Read(first_address++, 1, buffer++))
			return false;
	}
#endif 
	return true;
