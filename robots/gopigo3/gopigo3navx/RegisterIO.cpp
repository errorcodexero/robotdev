...
void RegisterIO::Run() {
    ...
    while (!stop) {
        ...
        GetCurrentData();
        delayMillis(static_cast<int>(update_rate_ms));
    }
}
...
void RegisterIO::GetCurrentData() {
    ...
    if ( displacement_registers ) {
        buffer_len = static_cast<uint8_t>(NAVX_REG_LAST + 1 - first_address);
    } else {
        buffer_len = static_cast<uint8_t>(NAVX_REG_QUAT_OFFSET_Z_H + 1 - first_address);
    }
